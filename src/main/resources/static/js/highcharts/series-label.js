...
(function(t) {
    ...
    (function(n) {
        ...
        function C(d) {
            var b = this,
                a = Math.max(n.animObject(b.renderer.globalAnimation).duration, 250),
                m = !b.hasRendered;
            ...
            w(b.series, function(c) {
                ...
                e.enabled && c.visible && (c.graph || c.area) && !c.isSeriesBoosting && (b.labelSeries.push(c), e.minFontSize && e.maxFontSize && (c.sum = n.reduce(c.yData, function(a, b) {
                    return (a || 0) + (b || 0)
                }, 0), b.labelSeriesMaxSum = Math.max(b.labelSeriesMaxSum, c.sum)), m && (a = Math.max(a, n.animObject(c.options.animation).duration)), h && (void 0 !== h[0].plotX ? d.animate({
                    x: h[0].plotX + h[1],
                    y: h[0].plotY + h[2]
                }) : d.attr({
                    opacity: 0
        ...
        F.prototype.symbols.connector = function(d, b, a, m, c) {
            var e = c && c.anchorX;
            c = c && c.anchorY;
            var f, h, g = a / 2;
        ...
        z.prototype.getPointsOnGraph = function() {
            if (this.xAxis || this.yAxis) {
                ...
                if (this.getPointSpline && f.getPointAtLength && !q) {
                    ...
                    if (0 < c && (e = Math.abs(b.chartX - a.chartX), f = Math.abs(b.chartY - a.chartY), e = Math.max(e, f), 16 < e)) for (e = Math.ceil(e / 16), f = 1; f < e; f += 1) m.push({
            ...
            for (p = 0; p < r.series.length; p += 1) if (u = r.series[p], k = u.interpolatedPoints, u.visible && k) {
                for (l = 1; l < k.length; l += 1) {
                    if (k[l].chartX >= d - 16 && k[l - 1].chartX <= d + a.width + 16) {
                        if (B(d, b, a.width, a.height, k[l - 1].chartX, k[l - 1].chartY, k[l].chartX, k[l].chartY)) return !1;
                    ...
                    if (a.visible && !a.isSeriesBoosting && p) {
                        ...
                        if (h.length) {
                            if (h.sort(function(a, b) {
                                ...
                                top: g.y,
                                bottom: g.y + c.height
                            }), h = Math.sqrt(Math.pow(Math.abs(g.x - r.x), 2), Math.pow(Math.abs(g.y - r.y), 2))) p = {
                                opacity: d.renderer.forExport ? 1 : 0,
                                x: g.x - n,
                                y: g.y - k
                            }, l = {
