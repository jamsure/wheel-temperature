...
jQuery(document).ready(function(){
	...
	//修改：点击左侧菜单栏房间，右侧控制台信息自动更改房间号及摄像头ip地址
		$("#room li").on('click',function (){
			$(this).siblings("li").removeClass("current");
			$(this).addClass("current");
            window.open('dashboard'+jQuery(this).text().toString().substr(0,1)+".html");
			// $("#pagedesc").text(jQuery(this).text());
			// $("#roomnumber").val(jQuery(this).text().toString().substr(2,1));  //输出当前房间号第三个字符
		});
